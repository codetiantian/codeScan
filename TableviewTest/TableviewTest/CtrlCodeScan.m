...
@interface CtrlCodeScan () <AVCaptureVideoDataOutputSampleBufferDelegate>
...
//    CGRect rect = output.rectOfInterest;
    ...
    line = [[UIView alloc] initWithFrame:CGRectMake(view.frame.origin.x + 1, view.frame.origin.y + 1, frame.size.width - 2, 2)];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    if ([self.delegate respondsToSelector:@selector(getCurrentImage:)]) {
        [self.delegate getCurrentImage:self.lastImage];
    }
...
//        
//        if ([ws.delegate respondsToSelector:@selector(getCurrentImage:)]) {
//            [ws.delegate getCurrentImage:image];
//        }
//    }];
}
...
#pragma mark - 使用AVCaptureVideoOutput实时获取预览图像
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection{
    ...
    self.lastImage = [self imageFromSampleBuffer:sampleBuffer];
}
